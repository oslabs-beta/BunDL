...
import { add, remove } from '../reducers/counterSlice';
...
function Fields() {
  ...
  const arr = ['firstName', 'lastName', 'email', 'city', 'state', 'country'];
  ...
  const handleClick = (index) => {
    ...
    if (currentButtonState.icon === faAdd && currentButtonState.backgroundColor === 'white') {
      ...
      dispatch(add(currentButtonState.value))
    }
    else {
      currentButtonState.icon = faAdd;
      currentButtonState.backgroundColor = 'white'
      dispatch(remove(currentButtonState.value))
    }
      ...
      <div className="schemaButtons">
        ...
        {arr.map((item, index) => {
          ...
          return (
            ...
            >
              <FontAwesomeIcon icon={buttonState.icon} style={{ color: 'black' }} />
              {item}
            </button>
          );
        })}
...
export default Fields;
           ...
           {/* <div className="addFieldsCircle">
                <div className="addFields">+</div>
              </div> */}